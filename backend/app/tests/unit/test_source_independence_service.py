...
def test_same_domain_search_results_are_discounted():
    evidence = [
        make_evidence(
            "E1",
            "https://www.espn.com/search/_/q/The+Boston+Team Green+won+the+2024+Example League+Finals.",
        ),
        make_evidence(
            "E2",
            "https://www.espn.com/search/_/q/The%20Boston%20Team Green%20won%20the%202024%20Example League%20Finals.",
        ),
    ]
...
def test_single_official_source_gets_high_independence():
    evidence = [
        make_evidence("E1", "https://www.example.org/playoffs/2024/example-final", 0.5),
    ]

    SourceIndependenceService().apply_to_evidence_items(evidence_items=evidence)

    assert evidence[0].independence == 0.85
    assert evidence[0].independence_group == "domain:example.org"
